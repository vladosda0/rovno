...
Implement backend truth changes in `rovno-db`.
...
migration first, then generation and validation as needed.

## Preconditions

Assume the parent should invoke this skill only after:
...
- if changing schema shape, ensure generator or contract outputs remain coherent
...
- membership, invites, or permissions
- RLS, grants, or policies
- RPCs, triggers, or auth bootstrap
