...
const EMPTY_CAN_USE_STOCK_MAP: ReadonlyMap<string, boolean> = new Map();

/** Frontend mirror of the backend `can_use_project_stock` gate: owner / co-owner, OR full
 *  financial-data access (sensitive detail). Used to filter cross-project stock-transfer
 *  destinations (the RPC enforces the same on both projects). */
function resolveCanUseStockForProject(
  project: Pick<Project, "id">,
  profileId: string,
  members: Member[],
  projectRow: Project | undefined,
  ...
  const effective = applyWorkspaceDemoOverlayToSeam(seam, workspaceMode);
  const role = getProjectRole(effective);
  return role === "owner" || role === "co_owner" || seamCanViewSensitiveDetail(effective);
}

/**
 * Per-project `can_use_project_stock` for all workspace projects. Same fan-out (members +
 * project rows) as the sensitive-detail map; fails closed while loading.
 */
export function useWorkspaceProjectsCanUseStockMap(): {
  canUseStockByProjectId: ReadonlyMap<string, boolean>;
  isLoading: boolean;
} {
  const workspaceMode = useWorkspaceMode();
  const workspaceHookProjects = useWorkspaceProjects();
  ...
  if (workspaceMode.kind === "pending-supabase") {
    return { canUseStockByProjectId: EMPTY_CAN_USE_STOCK_MAP, isLoading: true };
  }
  ...
  if (workspaceMode.kind === "demo" || workspaceMode.kind === "local") {
    ...
    for (const project of projects) {
      ...
      map.set(
        project.id,
        resolveCanUseStockForProject(project, user.id, members, projectRow, workspaceMode),
      );
    }
    return { canUseStockByProjectId: map, isLoading: false };
  }

  if (workspaceMode.kind !== "supabase") {
    return { canUseStockByProjectId: EMPTY_CAN_USE_STOCK_MAP, isLoading: false };
  }
